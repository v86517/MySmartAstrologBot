...
from .astrology_utils import calculate_aspects_manual  # <-- импорт общей функции
...
class TransitHoroscopeCalculator(BaseCalculator):
    ...
    # ---- УДАЛЁН метод _calculate_aspects_manual, вместо него используется импортированный calculate_aspects_manual ----
    ...
    def calculate(self) -> Dict[str, Any]:
        ...
        # ---- ИСПОЛЬЗУЕМ ОБЩУЮ ФУНКЦИЮ ДЛЯ АСПЕКТОВ ----
        transit_aspects = calculate_aspects_manual(natal_planets, transit_planets)
        ...
        for a in transit_aspects:
            if 'Moon' in a['p1'] or 'Moon' in a['p2']:
                moon_aspects.append(
                    f"Transit {a['p1']} → Natal {a['p2']} → {a['aspect']} → {a['orb']:.2f}°"
                )
        ...
        data = {
            ...
            "transit_aspects": "\n".join(
                f"Transit {a['p1']} → Natal {a['p2']} → {a['aspect']} → {a['orb']:.2f}°" for a in transit_aspects
