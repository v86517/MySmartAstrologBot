...
class YooKassaClient:
    ...
    def create_payment(self, user_id: int, amount: float, description: str, payment_type: str) -> dict:
        if not self.is_configured:
            return {'success': False, 'error': 'ЮKassa не настроена. Проверьте ключи в .env'}
        try:
            ...
            payment = Payment.create({
                ...
                "metadata": {"user_id": str(user_id), "type": payment_type}
            }, idempotence_key)
    ...
    def check_payment(self, payment_id: str) -> dict:
        ...
        try:
            ...
            return {'success': True, 'status': payment.status, 'paid': is_paid, 'payment': payment}
        except Exception as e:
            logger.error(f"❌ Ошибка проверки платежа {payment_id}: {e}")
            return {'success': False, 'error': str(e)}

    def capture_payment(self, payment_id: str, amount: float, currency: str = 'RUB') -> dict:
        if not self.is_configured:
            return {'success': False, 'error': 'ЮKassa не настроена'}
        try:
            ...
            logger.info(f"✅ Платёж {payment_id} захвачен на сумму {amount} {currency}")
            return {'success': True, 'payment': payment}
        except Exception as e:
            logger.error(f"❌ Ошибка захвата платежа {payment_id}: {e}")
            return {'success': False, 'error': str(e)}
